"""
Visualization helpers -- Code borrowed from
...
from acsm.utils.visuals import vis_utils


class VisRenderer(object):
    '''
    This classed is used to initialize the Pytorch NMR
    Only the constructor of this class is used. Other functionalities are used in the wrapper class in visdom_renderer.py
    ...
    def __init__(self, img_size, faces, t_size=3):
        ...
        mesh_color  = [0, 172, 223]     # blue
        # mesh_color  = [156, 199, 234.]  # light blue (use when overlaying with input image)
        blue        = np.array(mesh_color) / 255.
        default_tex = default_tex * blue

        # Could make each triangle different color
        self.default_tex = Variable(torch.FloatTensor(default_tex).cuda(), requires_grad=False)
        ...
        R1 = cv2.Rodrigues(np.array([0, np.pi /4 , 0]))[0]
        R  = R1.dot(R0)
        R  = np.vstack((np.hstack((R, np.zeros((3, 1)))), np.array([0, 0, 0, 1])))

        rot = transformations.quaternion_from_matrix(R, isprecise=True)
        cam = np.hstack([1., 0, 0, rot])
        self.default_cam = torch.FloatTensor(cam).cuda().unsqueeze(0)


    def __call__(self, verts, cams=None, texture=None, rend_mask=False):
        """
        Inputs:
        - verts : |V| x 3 (cuda torch Variable)
        - cams  : 7       (cuda torch Variable)
        Returns:
        - N x N x 3 numpy
        """
        if verts.dim() == 2:
        ...
        if texture is None:
            ...
            # Here input it F x T x T x T x 3 (instead of F x T x T x 3) => So add batch dim.
    ...
    def rotated(self, vert, deg, axis=[0, 1, 0], cam=None, texture=None):
        """
        vert : N x 3 (torch FloatTensor)
        """
        # get 3x3 rotation matrix from axis-anle representation
        new_rot  = cv2.Rodrigues(np.deg2rad(deg) * np.array(axis))[0]
        new_rot  = convert_as(torch.FloatTensor(new_rot), vert)
        center   = vert.mean(0)
        new_vert = torch.t(torch.matmul(new_rot, torch.t(vert - center))) + center  # rotate using rotation matrix
        return self.__call__(new_vert, cams=cam, texture=texture)


    def diff_vp(self, verts, cam=None, angle=90, axis=[1, 0, 0], texture=None, kp_verts=None, new_ext=None, extra_elev=False):
        if cam is None:
        ...
        R         = transformations.quaternion_matrix(quat.squeeze().data.cpu().numpy())[:3, :3]
        ...
        # Make homogeneous
        new_R    = np.vstack( [np.hstack((new_R, np.zeros((3, 1)))), np.array([0, 0, 0, 1])])
        new_quat = transformations.quaternion_from_matrix(new_R, isprecise=True)
        ...
        if kp_verts is None:
            ...
            kps = self.renderer.project_points(kp_verts.unsqueeze(0), new_cam.unsqueeze(0))
    ...
    for kpx, (keypoint, vis) in enumerate(zip(keypoints, keypoints_vis)):
        if vis > 0.05:
            ...
            if color_map is not None:
                color = color_map[kpx]
            img = cv2.circle(img,(int(keypoint[0].item()),int(keypoint[1].item())), 4, (color[0]*255,color[1]*255,color[2]*255), -1)
            img = cv2.putText(
                                img,
                                '{}_{:.2f}'.format(kp_names[kpx], vis),            # text
                                (int(keypoint[0].item()),int(keypoint[1].item())), # position at which writing has to start
                                cv2.FONT_HERSHEY_SIMPLEX,                          # font family
                                0.5,                                               # font size
                                (color[0]*255, color[1]*255, color[2]*255),        # font color
                                1                                                  # font stroke
            )
    return img
...
def save_obj_with_texture(name, results_path, texture_img, mean_shape):
    ...
    with open(mtl_file,'w') as f:
        ...
        f.write('illum 2\n')
        f.write('map_Kd {}.png\n'.format(name,))
    vis_utils.save_image(texture_img, osp.join(results_path,'{}.png'.format(name)))
    return
...
def render_model_with_uv_map(renderer, vert, uvimg_H, uvimg_W, camera, uv_sampler, tex_size=6, other_vps=False):
    texture_image = get_sample_grid((uvimg_H,uvimg_W)) * 0.5 + 0.5
...
def copy_texture_from_img(mask, img, xy_map):
    img = (vis_utils.undo_resnet_preprocess(img.unsqueeze(0))*mask).squeeze()
    ...
    new_img = vis_utils.tensor2im(new_img)
    return new_img
