'''
Code borrowed from
https://github.com/nileshkulkarni/acsm/blob/master/acsm/nnutils/uv_to_3d.py
'''
...
from acsm.model.utils.net_blocks import add_bIndex
...
class UVTo3D(nn.Module):
    ...
    def remove_component_perpendicular_to_face(self, verts, points):
        ...
        perp_direction = torch.nn.functional.normalize(perp_direction, dim=-1, p=2)
    ...
    def check_point_inside_triangle(self, triangle, points):
        matrixM = triangle.data.cpu().numpy().reshape(-1, triangle.shape[2], triangle.shape[3])
    ...
    def compute_barycentric_coordinates(self, uv_verts, uv_points):
        ...
        barycentric_coordinates = torch.nn.functional.normalize(barycentric_coordinates, p=1, dim=-1)
    ...
    def forward(self, verts, uv):
        ...
        facesWithBatch = add_bIndex(faces)
